...
\Cref{fig:platonic_solid}). As explained below, this angle can also be found in
...
The result is depicted in \Cref{fig:Quadcopter_1} and has the following features:
...
optimization tool has, as mentioned in \Cref{sec:limitations}, found a local
...
-35.26^{\circ}]$ as initial solution. The result in \Cref{fig:Quadcopter_1}
is defined by:
...
After this second optimization, the second design is found to be identical to the initial solution.
...
design form the four vertices of a tetrahedron (see \Cref{fig:Quadcopter_2_tetra}).\\
...
are equivalent. When comparing \Cref{fig:Quadcopter1_spaces} with
...
\Cref{tab:tab_Quad_compare_hover}, despite slightly different arm angles’
...
the optimal MAV design form is an octahedron (see \Cref{fig:Hexacopter_resultb}).
...
and in \Cref{fig:Voliro_spaces}, the first noticeable caracteristic is that the
...
around their axes (see \Cref{fig:Octacopter_resultb}). The Omnicopter's design
parameters are:
...
Even though the arm angles seem very peculiar in this optimal penta-copter, the
propellers appear to be quite evenly disposed around the MAV. Hence, when looking
at  \Cref{fig:penta_fspace} and \Cref{fig:penta_tspace} the force and torque spaces
of the optimal penta-copter are close to a sphere’s shape, and when looking at
\Cref{fig:penta_hspace} the hover efficiency displayed by this design seems outstanding.
And when examining the values showed in \Cref{tab:tab_Penta_compare_force},
...
And the information on their abilities is shown in \Cref{tab:tab_all_compare_force},
